"""Versioning and audit history for Execution Engine."""
...
class ExecutionHistory(FortiBaseModel):
    id: UUID = Field(default_factory=new_id)
    execution_id: UUID = Field(...)
    tenant_id: UUID = Field(...)
    plan_id: UUID = Field(...)
    version: int = Field(..., ge=1)
    snapshot: ExecutionResult = Field(...)
...
class ExecutionAuditRecord(FortiBaseModel):
    id: UUID = Field(default_factory=new_id)
